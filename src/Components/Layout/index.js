...
const Layout = () => {
  ...
  const allRestaurants = callApi(filterdRestaurants,setFilterdRestaurants);
...
//   },[]);
  ...
  //     const allRestaurants =  await fetch("https://www.swiggy.com/dapi/restaurants/list/v5?lat=12.9351929&lng=77.62448069999999&page_type=DESKTOP_WEB_LISTING");
    ...
    <>
      <div className='rounded-lg border-gray-400 relative shadow-lg w-4/5 mx-auto bg-indigo-500'>
        ...
        <button className=' absolute right-1 top-2 bg-transparent pr-1' onClick={() => {
          ...
          const data = filterData(searchText, filterdRestaurants);
